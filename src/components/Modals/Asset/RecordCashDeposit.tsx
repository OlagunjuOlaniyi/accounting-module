...
import { useRecordCashDeposit } from '../../../hooks/mutations/chartofAccounts';
import { useGetBankList } from '../../../hooks/queries/banks';

const RecordCashDeposit = ({ close }: any) => {
  ...
  const { mutate, isLoading } = useRecordCashDeposit();
  ...
  const submit = () => {
    ...
    mutate(dataToSend, {
      onSuccess: (res) => {
        close();
        toast.success('Transaction successfuls');
...
export default RecordCashDeposit;
