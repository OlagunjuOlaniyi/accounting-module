...
const EditProduct = ({
  ...
  useEffect(() => {
    setFields({
      ...
      same_unit_price: data?.data?.same_unit_price,
  ...
  const [existingProduct, setExisitingProduct] = useState(true);
  ...
  const [sameUnitPrice, setSameUnitPrice] = useState(
    data?.data?.same_unit_price
  );
  ...
  // console.log({ data });
  // console.log("same_unit_price",data?.data?.same_unit_price);
    ...
    >
      {isLoading ? (
        ...
        <div className="record-income">
          ...
          >
            {isEditLoading ? "Please wait..." : "Edit Product"}
          </button>
        </div>
      )}
    </Modal>
