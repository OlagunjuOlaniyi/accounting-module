...
const AddProduct = ({ modalIsOpen, closeModal }: Imodal) => {
  ...
  const handleSizePriceChange = (
    ...
    console.log("length");
    console.log(updatedSizePrices?.length);
    updatedSizePrices[index] = updatedSizePrice;
    setSizePrices(updatedSizePrices);
    // if (updatedSizePrices?.length > 1) {
    //   const isSizeAlreadyExists = updatedSizePrices.find(
    //     (item) => item.size === updatedSizePrice.size
    //   );

    //   if (isSizeAlreadyExists) {
    //     toast.error(`You cant add same size`);
    //     return;
    //   } else {
    //     updatedSizePrices[index] = updatedSizePrice;
    //     setSizePrices(updatedSizePrices);
