...
const RemitDeductions = ({ modalIsOpen, closeModal, id }: any) => {
    ...
    mutate(dataToSend, {
      onSuccess: (res) => {
        ...
        queryClient.invalidateQueries({
          queryKey: `allowance-and-deductions-${id}-DEDUCTION`,
        });
      },
      ...
      <div className='record-income'>
        <div style={{ background: '#FBFDFE' }}>
          ...
          <div className='record-income__heading'>
            <h4>Pay Deduction </h4>
            <p>
              Select the deduction type, payment method, and date of transaction
              of the deduction you want to pay
            </p>
          </div>
        </div>
        ...
        >
          {isLoading ? 'Please wait...' : 'Pay Deduction'}
        </button>
      </div>
    </Modal>
  );
...
export default RemitDeductions;
